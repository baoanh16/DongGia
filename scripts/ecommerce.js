	...
	function ProcessAjax(pageurl) {
		...
		$.ajax({
			url: pageurl, data: { isajax: true }, success: function (data) {
				...
				$('.productpager').remove();
				$(data).find('.productpager').insertAfter($('.ajaxresponsewrap'));
				$('.filter-wrapper .filter').html($(data)[1])
				$('.filter-wrapper .ajaxsortresponse').html($(data).find('.ajaxsortresponse'))
			}
		});
...
var AjaxCart = {
	...
	effecttocart: true,
	topcartselector: '.cart .cart-toggle .amount',
	topwishlistselector: '',
	...
	success_desktop: function (response) {
		var checkoutUrl = $('#checkout').attr('href')
		if (response.updatetopcartsectionhtml) {
			$(AjaxCart.topcartselector).html(response.updatetopcartsectionhtml);
		}
		if (response.updatetopwishlistsectionhtml) {
		...
		if (response.updateflyoutcartsectionhtml) {
			...
			// Edit here
			// $(AjaxCart.flyoutcartselector).find('.amount').html(response.updatetopcartsectionhtml)
			$('.cart .cartpanel').replaceWith($(response.updateflyoutcartsectionhtml)[2])
			$('.cart .cartpanel').addClass('open')
			$('.cart .backdrop').fadeToggle()
			// End Edit
			...
			$('#checkout').attr('onclick', "setLocation('" + checkoutUrl + "');return false;");
		}
		if (response.message) {
			//display notification
			if (response.success == true) {
